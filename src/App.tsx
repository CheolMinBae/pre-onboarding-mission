...
function App() {
  ...
  const highlightText = (text: string, highlight: string) => {
    ...
    return parts.map((part, index) => (part.toLowerCase() === highlight.toLowerCase() ? <strong key={index}>{part}</strong> : part));
  };

  const createList = (data: GroupedData) => {
  ...
  const listHandler = (description: string) => {
  ...
  return (
    <>
      ...
      {searchTerm && <div className="list-block">{createList(groupedData)}</div>}
    </>
  );
}
